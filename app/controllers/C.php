...

{
	...
	function google_oauth() {
        ...
        if($this->input->get('code')) {
            ...
            $params = [
                ...
                'redirect_uri' => base_url("c/google_oauth"),
            ...
            if(isset($adata['error'])) {
                ...
                curl_setopt($ch, CURLOPT_HTTPHEADER, ['Authorization: Bearer '.$adata['access_token']]);
    ...
    // Thêm method cho Facebook OAuth  
    ...
    function facebook_oauth() {
        ...
        if($this->input->get('code')) {
            ...
            $params = [
                ...
                'redirect_uri' => base_url("c/facebook_oauth")
            ...
            curl_setopt($ch, CURLOPT_URL, "https://graph.facebook.com/v12.0/oauth/access_token?" . http_build_query($params));
            ...
            if(isset($adata['error'])) {
                ...
                curl_setopt($ch, CURLOPT_URL, $this->oauth->get_endpoint($oauth) . "?fields=id,name,email&access_token=" . $adata['access_token']);
...
// Thêm phương thức xử lý Discord OAuth
function discord_oauth() {
    $this->load->model('user');
    $this->load->model('oauth');
    $oauth = 'discord';
    ...
    if($this->input->get('code')) {
        $params = [
            ...
            'grant_type' => 'authorization_code',
            'code' => $this->input->get('code'),
            'redirect_uri' => base_url("c/discord_oauth")
        ];
...
// Thêm phương thức xử lý Microsoft OAuth 
function microsoft_oauth() {
    $this->load->model('user');
    $this->load->model('oauth');
    $oauth = 'microsoft';
    ...
    if($this->input->get('code')) {
        $params = [
            ...
            'code' => $this->input->get('code'),
            'grant_type' => 'authorization_code',
            'redirect_uri' => base_url("c/microsoft_oauth"),
        ...
        if(isset($adata['error'])) {
            ...
            if(isset($udata['error'])) {
                ...
                $email = $udata['mail'] ?? $udata['userPrincipalName'];
